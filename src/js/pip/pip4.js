...
export class PiP4 {
  ...
  updateStencil(intersectionData, velocity = null, normalForce = 1.0, intensityScale = 1.0) {
    ...
    // Calculate line angle from velocity if provided
    if (velocity && velocity.x !== undefined && velocity.z !== undefined) {
      ...
      if (velocityMag > 0.1) {
        // Calculate angle perpendicular to velocity direction (tangent lines)
        // Use atan2 with velocity components and add 90 degrees for perpendicular
        let velocityAngle = Math.atan2(velocity.z, velocity.x) * (180 / Math.PI);
        this.lineAngle = velocityAngle;
        
        // Normalize angle to 0-360 range
        if (this.lineAngle < 0) this.lineAngle += 360;
        if (this.lineAngle >= 360) this.lineAngle -= 360;
        
        // Debug logging
        console.log(`Velocity: (${velocity.x.toFixed(2)}, ${velocity.z.toFixed(2)}), Angle: ${this.lineAngle.toFixed(1)}°`);
      }
    }
    ...
    // Calculate intensity directly from normal force with scale factor
    // Intensity Scale acts as a multiplier for the normal force
    const scaledForce = normalForce * intensityScale;
    const finalIntensity = Math.min(1.0, scaledForce / 50.0); // Normalize (50N = max intensity)
    ...
    if (this.useCustomPattern && this.customPatternImage) {
      ...
      // Debug logging for custom pattern rotation
      console.log(`Custom pattern rotation: reset to 0° (from ${this.customPatternDirection.toFixed(1)}°), then rotate by ${this.lineAngle.toFixed(1)}° (velocity)`);
      ...
      // Draw generated parallel lines at specified angle with intensity-based alpha
      lineCtx.strokeStyle = `rgba(255, 255, 255, ${finalIntensity})`;
      lineCtx.lineWidth = this.lineWidth;
      lineCtx.lineCap = 'round';
      
      const lineCount = Math.ceil(Math.sqrt(W * W + H * H) / this.lineSpacing);
      const angleRad = (this.lineAngle * Math.PI) / 180;
      
      for (let i = -lineCount; i <= lineCount; i++) {
        const offset = i * this.lineSpacing;
  ...
  detectImageDirection(image) {
    return new Promise((resolve) => {
      ...
      console.log('Image direction analysis:', gradients);
      console.log('Vertical continuity score:', verticalScore);
      console.log('Horizontal continuity score:', horizontalScore);
      console.log('Detected direction:', strongestDirection.name, `(${strongestDirection.angle}°)`);
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = async () => {
          ...
          console.log('Custom pattern loaded:', img.width, 'x', img.height);
          console.log('Detected pattern direction:', this.customPatternDirection + '°');
          resolve();
        };
        img.onerror = reject;
  ...
  forceUpdate(velocity = null, normalForce = 1.0, intensityScale = 1.0) {
    ...
    if (pip3Canvas) {
      ...
      this.updateStencil(intersectionData, velocity, normalForce, intensityScale);
      this.render();
    } else {
      // If no intersection data yet, just update with current parameters
      this.updateStencil(null, velocity, normalForce, intensityScale);
      this.render();
    }
  }
